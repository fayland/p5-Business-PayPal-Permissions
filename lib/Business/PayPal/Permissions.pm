...
sub new {
    ...
    $ua->default_header('X-PAYPAL-REQUEST-DATA-FORMAT', 'JSON'); ## JSON is more readable
...
sub RequestPermissions {
    ...
    my $res = $self->{ua}->post($self->{url} . "Permissions/RequestPermissions", Content => encode_json(\%x));
    return { error => [ { message => $res->status_line } ] } unless $res->is_success;
    my $data = decode_json($res->content);
    return $data unless $data->{token};

    # construct redirect_url
    my $url = 'https://www.paypal.com/cgi-bin/webscr?cmd=_grant-permission&request_token=' . $data->{token};
    if ($self->{sandbox}) {
        $url = 'https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_grant-permission&request_token=' . $data->{token};
    }
...
sub GetAccessToken {
    ...
    my $res = $self->{ua}->post($self->{url} . "Permissions/GetAccessToken", Content => encode_json(\%x));
    return { error => [ { message => $res->status_line } ] } unless $res->is_success;

    my $data = decode_json($res->content);
    $self->{__token} = $data->{token} if exists $data->{token};
    $self->{__tokenSecret} = $data->{tokenSecret} if exists $data->{tokenSecret};
...
sub GetBasicPersonalData {
    ...
    $attribute ||= ['http://axschema.org/contact/email', 'http://schema.openid.net/contact/fullname', 'https://www.paypal.com/webapps/auth/schema/payerID', 'http://axschema.org/namePerson/first', 'http://axschema.org/namePerson/last', 'http://openid.net/schema/company/name', 'http://axschema.org/contact/country/home'];
    ...
    my $AUTHORIZATION = x_pp_authorization_header($url, $self->{username}, $self->{password}, $token, $tokenSecret);
    ...
    my $res = $self->{ua}->post($url, Content => encode_json(\%x), 'Content-Type', 'application/json');
    return { error => [ { message => $res->status_line } ] } unless $res->is_success;
...
sub x_pp_authorization_header {
    my ($url, $api_user_id, $api_password, $access_token, $access_token_verifier) = @_;

    my $timestamp = time();
    my $signature = x_pp_authorization_signature($url, $api_user_id, $api_password, $timestamp, $access_token, $access_token_verifier);
    return "token=${access_token},signature=${signature},timestamp=${timestamp}";
}

sub x_pp_authorization_signature {
    my ($url, $api_user_id, $api_password, $timestamp, $access_token, $access_token_verifier) = @_;

    my $key = join('&', paypal_encode($api_password), paypal_encode($access_token_verifier));
    ...
    my $base = join('&', ("POST", paypal_encode($url), paypal_encode($sorted_query_string)));
